...
async def ask_2fa_key(client: Client, callback: CallbackQuery):
    ...
    await callback.answer("📝 Please enter your 2FA key now.", show_alert=False)
...
async def handle_2fa_key(client: Client, message: Message):
    ...
    # Only process if user is in the key entry flow
    ...
    if not is_valid_base32(key):
        ...
        del user_2fa_keys[user_id]  # Remove from flow on invalid key
        return

    try:
        # Validate the key by generating a TOTP
        totp = pyotp.TOTP(key)
        ...
        del user_2fa_keys[user_id]  # Remove from flow on error
...
async def generate_totp(client: Client, callback: CallbackQuery):
    ...
    if user_id not in user_2fa_keys or not user_2fa_keys[user_id]:
        await callback.answer("❌ No key found! Please enter your key first.", show_alert=True)
        await callback.message.edit_text(
    ...
    try:
        ...
        await callback.answer("❌ Error generating TOTP code.", show_alert=True)
        await callback.message.edit_text(
