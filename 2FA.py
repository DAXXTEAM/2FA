...
async def handle_2fa_key(client: Client, message: Message):
    ...
    # Only process if user is in the flow (has None entry from enter_2fa callback)
    if user_id not in user_2fa_keys or user_2fa_keys[user_id] is not None:
        return
